...
(() => {
  ...
  class W {
    constructor(e, t, s, i, a, r, n, o, h) {
      this.id = e, this.name = t, this.tooltip = s, this.itemText = i, this.cooldown = a, this.duration = r, this.energyCost = n, this.start = o, this.end = h, this.timer = 0, this.onCooldown = 0, this.active = 0, this.cooldownLeft = 0
    }
  }
  class q {
    constructor() {
  ...
  class Te {
    ...
    populate() {
      if (this.map = new ee, this.zombies = new Ae, this.humans = new Se, this.gameModel = ne.getInstance(), this.graveyard = new Oe, this.bullets = new rt, this.assaultStarted = 0, this.blasts = new nt, this.exclamations = new it, 0 == this.textures.length)
        for (let e = 0; e < 3; e++) {
          const t = [];
  ...
  class rt {
    ...
    updatePart(e, t) {
      fastDistance(e.x, e.y + 8, e.target.x, e.target.y) < e.hitbox ? (e.plague ? (this.zombies.inflictPlague(e.target), this.humans.damageHuman(e.target, e.damage)) : e.fireball ? (this.humans.burnHuman(e.target, e.damage), this.humans.damageHuman(e.target, e.damage)) : e.darkorb ? e.target.flags.dead || (this.humans.damageHuman(e.target, e.damage), e.target.timer.dogStun = 5, (new Xe).orbHit(e.target)) : !e.rocket && e.target.bulletReflect && Math.random() < e.target.bulletReflect ? this.newBullet(e.target, e.source, e.damage, 0, 0, 0) : e.rocket ? (e.target.graveyard && this.graveyard.damageGraveyard(e.damage), this.army.droneExplosion(e.target.x, e.target.y, null, e.damage)) : (e.target.zombie && this.zombies.damageZombie(e.target, e.damage, e.source), e.target.human && this.humans.damageHuman(e.target, e.damage)), e.visible = 0, this.discardedSprites.push(e), g.removeChild(e)) : (e.x += e.xSpeed * t, e.y += e.ySpeed * t, e.zIndex = e.y), e.darkorb ? e.alpha -= this.fadeSpeed * t * .4 : e.alpha -= this.fadeSpeed * t, e.alpha < 0 && (e.visible = 0, this.discardedSprites.push(e), g.removeChild(e))
    }
